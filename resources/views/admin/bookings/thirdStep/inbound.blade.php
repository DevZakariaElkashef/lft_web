...
<div class="row">
    {!! Form::open(['url' => ['javascript:void(0)'], 'method'=> null, 'id' => 'inbound_form', 'onclick' => 'inbound_submit_form()'  ]) !!}
    <div class="col-xl-4">
        <!--begin::Input-->
        <div class="form-group">
            {!! Form::label('containers_number_input', __('admin.containers_number')) !!}
            {!! Form::number('containers_number', (isset($booking) ?  ($booking->bookingContainers?->count() ?? old('containers_number')): old('containers_number')), ['id'=>'containers_number_input', 'class'=>'form-control', 'placeholder'=> __('admin.containers_number'), 'min'=>1]) !!}
    ...
    <div class="col-xl-4">
        ...
        <div class="form-group">
            ...
            {!! Form::select('factory_id', $factories, (isset($booking) ?  ($booking->factory_id ?? old('factory_id')): old('factory_id')), ['id' => 'factory_name_input', 'class' => 'form-control', 'placeholder' => __('admin.choose_factory')]) !!}
            @error('factory_id')
                <span class="form-text text-danger">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div class="col-xl-1">
        <!--begin::Select -->
        <div class="form-group">
            {!! Form::button(__('admin.add'), ['class' => 'btn btn-primary', 'id' => 'submit_inbound_form', 'onclick' => 'inbound_submit_form()']) !!}
        </div>
        <!--end::Select-->
    </div>
    {!! Form::close() !!}
    ...
    <table id="example" class="table table-striped" style="width:100%">
        ...
        <tbody id="tableRows">
            @if(isset($booking) && !is_null($booking->bookingContainers))
                ...
                <tr id="row_{{ $key }}" data-id="{{ $key }}">
                    <td>{{ $key + 1 }}</td>
                    <td>
                        {!! Form::text('sail_of_numbers[]', $bookingContainer->sail_of_number, ['class' => 'form-control', 'placeholder' => __('admin.navigational_torrent')]) !!}
                    </td>
                    <td>
                        {!! Form::text('container_no[]', $bookingContainer->container_no, ['class' => 'form-control', 'placeholder' => __('admin.container_no')]) !!}
                    </td>
                    <td>{{ $bookingContainer->container?->full_name }}</td>
                    <td>
                        {!! Form::select('branches[]', $branches, (isset($booking) ?  ($bookingContainer?->branch_id ?? old('branch_id')): old('branch_id')), ['id' => 'branch_name_input', 'class' => 'form-control branches', 'placeholder' => __('admin.choose_branch')]) !!}
                    </td>
                    <td>
                        {!! Form::date('arrival_dates[]', $bookingContainer->arrival_date, [ 'class' => 'form-control arrival_date_input', 'id' => 'arrival_date_input']) !!}
                    </td>
                </tr>
                @endforeach
            @else
                @if(!is_null(old('container_no')) || !is_null(old('arrival_date')))
                    ...
                    <tr id="row_{{ $i }}" data-id="{{ $i }}">
                        <input type="hidden" name="factoriesIDs[]" class="form-control" value="{{ old('factoriesIDs')[$i] }}">
                        <input type="hidden" name="containers[]" class="form-control" value="{{old('containers')[$i] }}">
                        <td>{{ $i + 1 }}</td>
                        <td>
                            {!! Form::text('sail_of_numbers[]', !is_null(old('sail_of_numbers')) ? old('sail_of_numbers')[$i] : null, ['class' => 'form-control', 'placeholder' => __('admin.navigational_torrent')]) !!}
                            @error('sail_of_numbers.'.$i)
                                <span class="form-text text-danger">{{ $message }}</span>x
                            @enderror
                        </td>
                        <td>
                            {!! Form::text('container_no[]', !is_null(old('container_no')) ? old('container_no')[$i] : null, ['class' => 'form-control', 'placeholder' => __('admin.container_no')]) !!}
                        ...
                        <td>
                            {!! Form::select('branches[]', factoryBranches(old('factoriesIDs')[$i]), old('branches')[$i], ['id' => 'branch_name_input branches_'.$i, 'class' => 'form-control branches', 'placeholder' => __('admin.choose_branch')])  !!}
                            @error('branches.'.$i)
                                <span class="form-text text-danger">{{ $message }}</span>
                            @enderror
                        </td>
                        <td>
                            {!! Form::date('arrival_dates[]', !is_null(old('arrival_dates')) ? old('arrival_dates')[$i] : null, [ 'class' => 'form-control arrival_date_input', 'id' => 'arrival_date_input']) !!}
